...
@Command(name = "statecraft", description = "Run Statecraft commands.", mixinStandardHelpOptions = true, version = StatecraftCli.VERSION, subcommands = {
		StatecraftCli.Engines.class, StatecraftCli.Demo.class, StatecraftCli.Run.class, StatecraftCli.Suite.class})
public final class StatecraftCli implements Callable<Integer> {
	...
	static final class Demo implements Callable<Integer> {
		...
		public Integer call() {
			...
			var out = spec.commandLine().getOut();
			result.finalState().ifPresentOrElse(state -> {
		...
		private void validateOptions() {
			...
			if (samples && shots == 0) {
				throw new CommandLine.ParameterException(commandLine, "--samples requires --shots to be provided");
			}
			if (seed != null && shots == 0) {
				throw new CommandLine.ParameterException(commandLine, "--seed requires --shots to be provided");
			}
	...
	static final class NoiseOptions {
		...
		private NoiseOptionsSupport.ResolvedNoiseSpec resolve(CommandLine commandLine) {
			NoiseOptionsSupport.NoiseOptionsInput input = new NoiseOptionsSupport.NoiseOptionsInput(noiseConfig,
					noiseSeed, noiseDepolarizing, noiseAmplitudeDamping, noisePhaseFlip, noisePhaseDamping,
					noiseThermalT1, noiseThermalT2, noiseThermalGateTime);
	...
	private static void printMeasurement(PrintWriter out, MeasurementResult measurement) {
		...
		if (measurement instanceof MeasurementResult.Histogram histogram) {
			...
			histogram.counts().entrySet().stream().sorted(java.util.Map.Entry.comparingByKey()).forEach(entry -> {
			...
			out.println("Shot histogram (qubits " + qubitLabel + "):");
			histogram.counts().entrySet().stream().sorted(java.util.Map.Entry.comparingByKey())
	...
	private static String formatAmplitude(double real, double imag) {
		...
		return String.format(Locale.US, "%.6f %s %.6fi", real, imag >= 0.0 ? "+" : "-", Math.abs(imag));
	}

 ...
 * <p>
 * A quantum channel E maps density matrices as: E(ρ) = Σ_i K_i ρ K_i†
 * where the Kraus operators satisfy the completeness relation: Σ_i K_i† K_i = I
 * <p>
 * For Monte Carlo simulation, we stochastically select one Kraus operator
 * based on the probabilities and apply it to the state vector.
 *
 * @param operators list of Kraus operators with their selection probabilities
 * @param numQubits number of qubits this channel acts on
 */
public record KrausDecomposition(List<KrausOperator> operators, int numQubits) {
    ...
    public KrausDecomposition {
        ...
        for (KrausOperator op : operators) {
            if (op.dimension() != expectedDim) {
                throw new IllegalArgumentException(
                        "all operators must have dimension " + expectedDim + ", got "
                                + op.dimension());
            }
        }
        ...
        if (Math.abs(sumProb - 1.0) > COMPLETENESS_TOLERANCE) {
            throw new IllegalArgumentException(
                    "operator probabilities must sum to 1.0, got " + sumProb);
        }
    }
     ...
     *
     * @throws IllegalStateException if the completeness relation is violated
     */
            ...
            for (int col = 0; col < dim; col++) {
                ...
                if (realDiff > COMPLETENESS_TOLERANCE || imagDiff > COMPLETENESS_TOLERANCE) {
                    throw new IllegalStateException(
                            "completeness relation violated at (" + row + "," + col + "): expected "
                                    + expected + ", got " + actual);
                }
            }
        }

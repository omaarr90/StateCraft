 ...
 * circuits, including depolarizing errors, amplitude damping, phase errors, and
...
public sealed interface ErrorChannel permits DepolarizingChannel, AmplitudeDampingChannel, PhaseFlipChannel,
	 ...
	 * effectively randomizing the qubit state. With probability (1-p), the qubit is
	 * left unchanged.
	 *
	 * @param probability
	 *            error probability in [0,1]
	 * @param qubits
	 *            target qubits (currently only single-qubit supported)
	 ...
	 * <p>
	 * Models the |1⟩ → |0⟩ decay process with damping parameter γ. Represents
	 ...
	 * <p>
	 * With probability p, applies a Z gate (phase flip). Otherwise leaves the qubit
	 * unchanged. This models random phase errors without affecting population.
	 *
	 * @param probability
	 ...
	 * <p>
	 * Models loss of quantum coherence without energy relaxation. The parameter λ
	 * controls the strength of dephasing.
	 *
	 * @param lambda
	 ...
	 * <p>
	 * Models realistic qubit decoherence including both energy relaxation (T₁) and
	 * dephasing (T₂). The gate time determines how much decoherence accumulates
	 ...
	 * <p>
	 * The channels are applied in the order provided. This allows building complex
	 * noise models from simpler components.
	 *
	 * @param channels
	 *            channels to compose (must be non-empty)

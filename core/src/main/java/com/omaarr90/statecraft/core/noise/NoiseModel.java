 ...
 * 		.afterAllGates(ErrorChannel.thermalRelaxation(50e-6, 30e-6, 100e-9, 0)).build();
 * }</pre>
 * <p>
...
public final class NoiseModel {
	...
	// Idle-time decoherence templates on specific qubits (qubit -> list of
	// channels)
	...
	/**
	 * Legacy constructor for backward compatibility. Creates an empty noise model
	 * that can be configured using fluent methods.
	 * <p>
	 ...
	 * compatibility. For new code, prefer using
	 * {@link Builder#afterGate(Class, ErrorChannel)}.
	 *
	 * @param gateType
	 *            gate operation class
	 * @param channel
	 *            error channel to apply
	 ...
	 * Note: This method mutates the NoiseModel and is provided for backward
	 * compatibility. For new code, prefer using
	 * {@link Builder#onQubits(ErrorChannel, int...)}.
	 *
	 * @param channel
	 *            error channel to apply
	 * @param qubits
	 ...
	 * Note: This method mutates the NoiseModel and is provided for backward
	 * compatibility. For new code, prefer using
	 * {@link Builder#afterAllGates(ErrorChannel)}.
	 *
	 * @param channel
	 *            error channel to apply
	 * @return this model for chaining
	...
	private static void validateIdleChannel(ErrorChannel channel) {
		if (!isSingleQubitChannel(channel)) {
			throw new IllegalArgumentException("onQubits requires channels that decompose to single-qubit operators");
		}
	}
	 ...
	 * <p>
	 * Provides a fluent API for configuring noise channels with validation and
	 * defensive copying to ensure immutability of the final model.
	 */
	public static final class Builder {
		 ...
		 *             if gateType or channel is null
		 */
		 ...
		 *            error channel to apply
		 * @param qubits
		 *            target qubits (must be non-empty)
		 * @return this builder for chaining
		 * @throws NullPointerException
		 *             if channel or qubits is null
		 * @throws IllegalArgumentException
		 *             if qubits array is empty
		 */
		 ...
		 * <p>
		 * The returned model is completely independent from this builder; further
		 * modifications to the builder will not affect the returned model.
		 *
	...
	public String toString() {
		return "NoiseModel[gateTypes=" + gateNoise.keySet().size() + ", qubits=" + qubitNoise.keySet().size()
				+ ", global=" + globalNoise.size() + "]";
	}
}

...
final class DepolarizingChannel implements ErrorChannel {
    ...
    DepolarizingChannel(double probability, int... qubits) {
        ...
        if (qubits.length != 1) {
            throw new IllegalArgumentException(
                    "depolarizing channel currently only supports single qubits");
        }
    ...
    private KrausDecomposition buildKrausDecomposition() {
        ...
        KrausOperator k3 = ErrorChannel.singleQubitOperator(
                pEach,
                sqrtPEach,
                ComplexNumber.zero(),
                ComplexNumber.zero(),
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof DepolarizingChannel other)) return false;

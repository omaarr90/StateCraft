 ...
 * <p>
 * With probability p, applies a Z gate (phase flip).
 * With probability (1-p), leaves the qubit unchanged.
 * <p>
 * This models random phase errors that preserve population but
 * destroy superpositions by flipping the relative phase.
 * <p>
 * Kraus operators:
 * <ul>
...
final class PhaseFlipChannel implements ErrorChannel {
    ...
    PhaseFlipChannel(double probability, int... qubits) {
        ...
        if (qubits.length != 1) {
            throw new IllegalArgumentException(
                    "phase flip channel currently only supports single qubits");
        }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof PhaseFlipChannel other)) return false;

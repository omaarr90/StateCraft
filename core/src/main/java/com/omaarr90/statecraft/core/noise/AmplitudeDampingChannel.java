 ...
 * <p>
 * Models the |1⟩ → |0⟩ decay process characteristic of T₁ relaxation.
 * The damping parameter γ represents the probability of energy loss.
 * <p>
 * Kraus operators:
 * <ul>
 *   <li>K₀ = [[1, 0], [0, √(1-γ)]]</li>
...
final class AmplitudeDampingChannel implements ErrorChannel {
    ...
    AmplitudeDampingChannel(double gamma, int... qubits) {
        ...
        if (qubits.length != 1) {
            throw new IllegalArgumentException(
                    "amplitude damping channel currently only supports single qubits");
        }
        this.gamma = gamma;
    ...
    private KrausDecomposition buildKrausDecomposition() {
        ...
        KrausOperator k0 = ErrorChannel.singleQubitOperator(
                1.0 - gamma,  // Probability that no decay occurs
                ComplexNumber.one(),
                ComplexNumber.zero(),
                ComplexNumber.zero(),
                sqrtOneMinusGamma);
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof AmplitudeDampingChannel other)) return false;
        return Double.compare(gamma, other.gamma) == 0
    ...
    public String toString() {
        return "AmplitudeDampingChannel[γ=" + gamma + ", qubits="

...
public class ComplexArrays {
	...
	private static int toDoubleIndex(int offC) {
		return offC << 1;
	}
...
/// See preconditions in norm2(double[], int, int).
	 ...
	 *
	 * Layout: AoS interleaved [re0, im0, re1, im1, …]. Offsets and lengths are in
	 * complex elements, not doubles.
	 *
	...
	public static void scal(double[] x, int xOffC, int nC, double alphaRe, double alphaIm) {
		...
		if (nC < 0)
			throw new IllegalArgumentException("nC must be non-negative");
	 ...
	 * complex elements.
	 *
	 * Aliasing: correct even when x and y are the same array or overlapping
	...
	/**
	 * Elementwise complex multiply: out[i] ← x[i] * y[i]. Supports in-place out==x
	 * or out==y.
	 *
	 * Layout: AoS interleaved [re0, im0, re1, im1, …]. Offsets and lengths are in
	 * complex elements.
	 *
	 * Aliasing: out may be the same array slice as either x or y. It must not alias
	 * both inputs simultaneously unless x and y refer to the exact same slice (same
	 * array and offsets). In that unsupported case, an IllegalArgumentException is
	 * thrown.
	 *

...
/**
 * Phase damping (dephasing) channel modeling pure dephasing without energy loss.
 * <p>
 * Models loss of quantum coherence that affects the off-diagonal elements
 * of the density matrix without changing the populations.
 * <p>
 * Kraus operators:
 * <ul>
 *   <li>K₀ = [[1, 0], [0, √(1-λ)]]</li>
...
final class PhaseDampingChannel implements ErrorChannel {
    ...
    PhaseDampingChannel(double lambda, int... qubits) {
        ...
        if (qubits.length != 1) {
            throw new IllegalArgumentException(
                    "phase damping channel currently only supports single qubits");
        }
        this.lambda = lambda;
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof PhaseDampingChannel other)) return false;
        return Double.compare(lambda, other.lambda) == 0

...
class StabilizerRandomCliffordConformanceTest {
    ...
    void randomCliffordCircuitsMatchStatevector() {
        ...
        for (int trial = 0; trial < 24; trial++) {
            ...
            StateVector expected = statevector.simulate(SimulationRequest.zeroState(circuit))
                    .finalState()
                    .orElseThrow();
            StateVector actual = stabilizer.simulate(SimulationRequest.zeroState(circuit))
                    .finalState()
                    .orElseThrow();
            ...
            MeasurementResult.Histogram expectedHistogram = (MeasurementResult.Histogram) statevector
                    .simulate(request)
                    .measurement()
                    .orElseThrow();
            MeasurementResult.Histogram actualHistogram = (MeasurementResult.Histogram) stabilizer
                    .simulate(request)
                    .measurement()
                    .orElseThrow();

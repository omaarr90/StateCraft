 ...
 *
 * <p>
 * Given an m x n complex matrix C = X + iY, we form the real block matrix:
 *
 * <pre>
 * [ X  -Y ]
 * [ Y   X ]
...
final class EjmlComplexSvdAdapter implements ComplexSvdAdapter {
	...
	public SvdSplit split(int rows, int cols, double[] matrixData, int maxRank, double singularCutoff) {
		...
		SingularValueDecomposition_F64<DMatrixRMaj> svd = DecompositionFactory_DDRM.svd(realified.numRows,
				realified.numCols, false, true, true);
	...
	private static List<Mode> recoverComplexModes(int rows, int cols, double[] matrixData, double[] singularValues,
			int svCount, DMatrixRMaj vReal, int complexModes) {
		...
		while (start < svCount && modes.size() < complexModes) {
			...
			int groupAdded = 0;
			for (int column = start; column < end && modes.size() < complexModes
					&& groupAdded < groupTarget; column++) {

...
final class StabilizerTableau {
	...
	private StabilizerTableau(int qubitCount, int rowCount, int wordCount, long[] x, long[] z, boolean[] phase) {
		this.qubitCount = qubitCount;
	...
	StabilizerTableau copy() {
		return new StabilizerTableau(qubitCount, rowCount, wordCount, x.clone(), z.clone(), phase.clone());
	}
	...
	private void rowSum(int targetRow, int sourceRow) {
		...
		for (int qubit = 0; qubit < qubitCount; qubit++) {
			phaseAccumulator += phaseContribution(getX(sourceRow, qubit), getZ(sourceRow, qubit),
					getX(targetRow, qubit), getZ(targetRow, qubit));
		}

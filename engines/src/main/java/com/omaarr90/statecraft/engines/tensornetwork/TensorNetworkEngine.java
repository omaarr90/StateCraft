...
public final class TensorNetworkEngine implements SimulatorEngine {
	...
	private MpsState createInitialState(SimulationRequest request) {
		if (request.initialState().isPresent()) {
			...
			if (stateVector.qubitCount() > MAX_INITIAL_STATE_QUBITS) {
				throw new UnsupportedOperationException("tensornetwork engine v1 supports dense initial states up to "
		...
		if (request.basisStateQubits().isPresent()) {
			return MpsState.basisState(request.circuit().qubitCount(), request.basisStateQubits().orElseThrow(),
					svdAdapter, maxBondDimension, singularCutoff);
		}
	...
	private static StateVector materializeFinalState(MpsState state, int qubitCount) {
		if (qubitCount > MAX_FINAL_STATE_QUBITS) {
			throw new UnsupportedOperationException("tensornetwork engine cannot materialize amplitudes above "
	...
	private static void applyOperation(MpsState state, QuantumCircuit.Operation operation) {
		...
		if (operation instanceof QuantumCircuit.Operation.MultiControlOperation) {
			throw new UnsupportedOperationException("tensornetwork engine v1 does not support multi-control gates");
		}
		throw new UnsupportedOperationException(
	...
	private static MeasurementResult performMeasurement(MpsState state, int totalQubits,
	...
	private static MeasurementResult sampleSmallMeasurement(MpsState state, MeasurementInstruction instruction,
			int[] measuredQubits, SplittableRandom rng) {
	...
	private static MeasurementResult sampleBitstringMeasurement(MpsState state, MeasurementInstruction instruction,
			int[] measuredQubits, SplittableRandom rng) {
		int shots = instruction.shots();
		int[] bitIndexByPhysical = buildBitIndexByPhysical(state, measuredQubits);
		double[][] rightEnvironments = buildRightEnvironments(state);
	...
	private static int[] sampleShotBits(MpsState state, int measuredQubitCount, int[] bitIndexByPhysical,
			double[][] rightEnvironments, SplittableRandom rng) {
	...
	private static void ensureNoNoise(SimulationRequest request) {
		request.noiseModel().ifPresent(model -> {
			if (model.hasNoise()) {
				throw new UnsupportedOperationException("tensornetwork engine does not support noisy simulation yet");
			}
		});
	}
	...
	private static void ensureSupportedQubitCount(int qubitCount) {
		if (qubitCount > MAX_QUBITS) {
			throw new UnsupportedOperationException(
					"tensornetwork engine supports up to " + MAX_QUBITS + " qubits, got " + qubitCount);
		}
	}
	...
	private static void ensureShallowDepth(QuantumCircuit circuit) {
		...
		if (depth > MAX_DEPTH) {
			throw new UnsupportedOperationException(
					"tensornetwork engine supports shallow circuits only (depth <= " + MAX_DEPTH + "), got " + depth);
		}
	}

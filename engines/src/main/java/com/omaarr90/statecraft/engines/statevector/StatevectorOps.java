...
public final class StatevectorOps {
    ...
    private static final VectorShuffle<Double> SWAP_SHUFFLE = VECTOR_COMPLEX_AVAILABLE
            ? buildSwapShuffle()
            : null;
    private static final DoubleVector SIGN_VECTOR = VECTOR_COMPLEX_AVAILABLE
            ? buildSignVector()
            : null;
    ...
    public static void applySingleGate(double[] state, int target,
        ...
        for (int base = 0; base < dimension; base += period) {
            ...
            for (; offset < vectorLimit; offset += COMPLEX_PER_VECTOR) {
                ...
                DoubleVector new0 = complexMul(a0, g00rVec, g00iVec)
                        .add(complexMul(a1, g01rVec, g01iVec));
                DoubleVector new1 = complexMul(a0, g10rVec, g10iVec)
                        .add(complexMul(a1, g11rVec, g11iVec));
        ...
        for (int baseHigh = 0; baseHigh < dimension; baseHigh += blockSize) {
            for (int baseMid = 0; baseMid < highStride; baseMid += betweenStep) {
                ...
                for (; offset < vectorLimit; offset += COMPLEX_PER_VECTOR) {
                    ...
                    DoubleVector new0 = complexMul(a00, m00rVec, m00iVec)
                            .add(complexMul(a01, m01rVec, m01iVec))
                            .add(complexMul(a10, m02rVec, m02iVec))
                            .add(complexMul(a11, m03rVec, m03iVec));
                    DoubleVector new1 = complexMul(a00, m10rVec, m10iVec)
                            .add(complexMul(a01, m11rVec, m11iVec))
                            .add(complexMul(a10, m12rVec, m12iVec))
                            .add(complexMul(a11, m13rVec, m13iVec));
                    DoubleVector new2 = complexMul(a00, m20rVec, m20iVec)
                            .add(complexMul(a01, m21rVec, m21iVec))
                            .add(complexMul(a10, m22rVec, m22iVec))
                            .add(complexMul(a11, m23rVec, m23iVec));
                    DoubleVector new3 = complexMul(a00, m30rVec, m30iVec)
                            .add(complexMul(a01, m31rVec, m31iVec))
                            .add(complexMul(a10, m32rVec, m32iVec))
                            .add(complexMul(a11, m33rVec, m33iVec));
                ...
                for (; offset < lowStride; offset++) {
                    ...
                    double new00r = m00r * a00r - m00i * a00i
                            + m01r * a01r - m01i * a01i
                            + m02r * a10r - m02i * a10i
                            + m03r * a11r - m03i * a11i;
                    double new00i = m00r * a00i + m00i * a00r
                            + m01r * a01i + m01i * a01r
                            + m02r * a10i + m02i * a10r
                            + m03r * a11i + m03i * a11r;

                    double new01r = m10r * a00r - m10i * a00i
                            + m11r * a01r - m11i * a01i
                            + m12r * a10r - m12i * a10i
                            + m13r * a11r - m13i * a11i;
                    double new01i = m10r * a00i + m10i * a00r
                            + m11r * a01i + m11i * a01r
                            + m12r * a10i + m12i * a10r
                            + m13r * a11i + m13i * a11r;

                    double new10r = m20r * a00r - m20i * a00i
                            + m21r * a01r - m21i * a01i
                            + m22r * a10r - m22i * a10i
                            + m23r * a11r - m23i * a11i;
                    double new10i = m20r * a00i + m20i * a00r
                            + m21r * a01i + m21i * a01r
                            + m22r * a10i + m22i * a10r
                            + m23r * a11i + m23i * a11r;

                    double new11r = m30r * a00r - m30i * a00i
                            + m31r * a01r - m31i * a01i
                            + m32r * a10r - m32i * a10i
                            + m33r * a11r - m33i * a11i;
                    double new11i = m30r * a00i + m30i * a00r
                            + m31r * a01i + m31i * a01r
                            + m32r * a10i + m32i * a10r
                            + m33r * a11i + m33i * a11r;

...
final class MpsState {
    ...
    private MpsState(
            int qubitCount,
            ComplexSvdAdapter svdAdapter,
            int maxBondDimension,
    ...
    static MpsState fromStateVector(
        ...
        for (int site = 0; site < qubitCount; site++) {
            ...
            ComplexSvdAdapter.SvdSplit split = svdAdapter.split(
                    matrixRows, rightColumns, matrix, maxBondDimension, singularCutoff);
    ...
    void applyDiagonal(int firstLogicalQubit, int secondLogicalQubit, ComplexNumber[] diagonal) {
        applyTwoQubitOperation(
                firstLogicalQubit,
                secondLogicalQubit,
                TensorNetworkMatrices.diagonalToMatrix(diagonal));
    }
    ...
    private static ComplexSvdAdapter.SvdSplit fallbackTruncatedSplit(
            int rows,
            int cols,
            double[] matrix,
            int maxRank) {
